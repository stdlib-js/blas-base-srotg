...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( srotg.assign.length, 5, 'arity of 5' );
	t.end();
});
...
tape( 'the function computes a Givens plane rotation', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		...
		srotg.assign( float64ToFloat32( values[i][0] ), float64ToFloat32( values[i][1] ), out, 1, 0 ); // eslint-disable-line max-len
...
tape( 'the function supports providing a positive stride', function test(t) {
	...
	actual = srotg.assign( 0.3, 0.4, out, 2, 0 );
...
tape( 'the function supports providing a negative stride', function test(t) {
	...
	actual = srotg.assign( 0.3, 0.4, out, -2, 6 );
	t.deepEqual( out, expected, 'returns expected value' );
	t.deepEqual( actual, expected, 'returns expected value');
	t.end();
});
...
tape( 'the function supports providing a positive offset', function test(t) {
	...
	actual = srotg.assign( 0.3, 0.4, out, 1, 1 );
	t.deepEqual( out, expected, 'returns expected value' );
	t.deepEqual( actual, expected, 'returns expected value');
	t.end();
});

tape( 'the function supports providing both stride and offset', function test(t) {
	var expected;
	var actual;
	var out;
	...
	actual = srotg.assign( 0.3, 0.4, out, 2, 2 );
	t.deepEqual( out, expected, 'returns expected value' );
	t.deepEqual( actual, expected, 'returns expected value');
	t.end();
});
